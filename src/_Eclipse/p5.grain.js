...
export class P5Grain {
    ...
    textureOverlay(textureImage, config, pg) {
        ...
        const _width = isGraphicsBuffer ? pg.width : (this.instance ? this.instance.width : 100);
        // height of the canvas or context
        const _height = isGraphicsBuffer ? pg.height : (this.instance ? this.instance.height : 100);
        // blend mode used to blend the texture over the canvas or context
        const _mode = config && config.mode ? config.mode : (this.instance ? this.instance.MULTIPLY : p5.MULTIPLY);
        // should reflect flag
        ...
        const _animateAtFrame = (
            ...
            ? (this.instance ? this.instance.round(config.animate.atFrame) : 1)
            : 2
        );
        // animate amount
        const _animateAmount = (
            config && config.animate && config.animate.amount
            ? (this.instance ? this.instance.round(config.animate.amount) : 1)
            : (this.instance ? this.instance.min(_width, _height) : 100)
        );
        // texture width
        ...
        let tColFirst = true;
        pg ? pg.blendMode(_mode) : (this.instance ? this.instance.blendMode(_mode) : 'none');
        while (tY < _height) {
            while (tX < _width) {
                if (_reflect) {
                    if (!isGraphicsBuffer) {
        ...
        // reset blend mode
        pg 
        ? pg.blendMode(this.instance ? this.instance.BLEND : 'none')
        : (this.instance ? this.instance.blendMode(this.instance.BLEND) : 'none');
        // reset context
        if (isGraphicsBuffer) {
            pg.reset();
        }
